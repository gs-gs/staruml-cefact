...
function savePackageToFile (package,filename) {
    try {
      // Build intermediate JSON representations
      var project = app.project.getProject()
      var root = {
    ...
    //   writeElementArray(root, 'packagedElement', project.ownedElements)
    // var fun = elements[package.getClassName()]
    // if (fun) {
    //   var node = fun(package)
    //   addTo(json, name, node)
    // }
    //   deferedNodes.forEach(function (node) {
        addTo(root, 'packagedElement', package)
    //   })
...
exports.savePackageToFile = savePackageToFile